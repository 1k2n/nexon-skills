...
const AILab = () => {
    ...
    const plannedSkills = useMemo(() => ([
        { id: 'node_0', phase: 'plan', skill: 'normalize_kpi_request', skillName: 'KPI Planner', skillType: 'Agent', route: 'skills://planner-router', output: 'analysis_window=2026-02-13~2026-02-19', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-AG-0042', detail: '유저 입력을 KPI 쿼리/기간/채널 파라미터로 정규화합니다.', icon: Cpu },
        { id: 'node_1', phase: 'execute', skill: 'daily_revenue_impact_search', skillName: 'AI 서치 에이전트', skillType: 'Agent', route: 'skills:ai-search-agent', output: '일매출 +8.4 %, 퀘스트 기여매출 1.98억 원', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-AG-0108', detail: '매출 로그와 이벤트 테이블을 탐색해 일별 영향도를 계산합니다.', icon: Database },
        { id: 'node_2', phase: 'execute', skill: 'ccu_variation_tracking', skillName: 'NEMO RT 에이전트', skillType: 'Connector', route: 'skills:nemort-agent', output: '평균 동접 +11.7 %, 피크 동접 +14.2 %', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-CN-0033', detail: 'CCU 스트림을 추적해서 평균/피크 변동량을 산출합니다.', icon: Activity },
        { id: 'node_5', phase: 'merge', skill: 'merge_daily_kpi', skillName: 'Daily KPI Joiner', skillType: 'Workflow', route: 'skills://runtime/instant-merge-kpi', output: '전날 종합 브리프(JSON) 생성 완료', origin: 'instant', originLabel: '인스턴트 스킬 (자동 생성)', catalogId: null, detail: '이번 실행에서만 생성된 임시 스킬. 매출/동접 결과를 브리프 포맷으로 병합합니다.', icon: GitBranch },
        { id: 'node_3', phase: 'deliver', skill: 'compose_email_digest', skillName: 'Email Summary', skillType: 'Connector', route: 'skills:email-summary', output: '기획팀 메일 발송 성공 (09:00 KST)', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-CN-0077', detail: '브리프를 이메일 템플릿으로 렌더링해 예약 발송합니다.', icon: Mail },
        { id: 'node_4', phase: 'deliver', skill: 'publish_slack_digest', skillName: 'Slack Notifier', skillType: 'Connector', route: 'skills:slack-notifier', output: '#planning-daily 채널 전송 성공 (09:00 KST)', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-CN-0091', detail: '브리프 요약을 슬랙 채널 카드 메시지 형태로 게시합니다.', icon: MessageSquare },
        { id: 'node_6', phase: 'audit', skill: 'archive_delivery_status', skillName: 'Delivery Audit', skillType: 'Agent', route: 'skills://ops-audit-log', output: 'delivery_id=DLV-20260220-090001', origin: 'catalog', originLabel: '넥슨스킬즈 라이브러리', catalogId: 'NS-AG-0156', detail: '각 전송 결과를 감사 로그 저장소에 기록합니다.', icon: FileText },
    ]), []);
    ...
    const phaseSteps = [
        ...
        { num: 2, label: '서브 에이전트 실행', icon: Zap, color: 'amber' },
        { num: 3, label: 'DIFY GUI 디테일링', icon: Layers, color: 'blue' },
    ];
    const phaseStatus = (n) => {
        if (n === 1) return phase >= 2 ? 'done' : phase === 1 ? 'active' : 'idle';
        if (n === 2) return phase >= 4 ? 'done' : phase === 3 ? 'active' : phase === 2 ? 'ready' : 'idle';
        if (n === 3) return phase >= 4 ? 'ready' : 'idle';
        return 'idle';
    };
    const tagColor = (tag) => {
    ...
    return (
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-3 text-slate-600 leading-relaxed text-sm">
                            복잡한 자연어 명령을 <strong className="text-violet-700">오케스트레이터</strong>가 1차 분석 → <strong className="text-violet-700">DIFY YAML</strong>로 변환하고,
                            넥슨스킬즈 카탈로그의 스킬을 <strong className="text-violet-700">서브 에이전트</strong>가 실행한 뒤, <strong className="text-violet-700">DIFY GUI</strong>에서 세부 디테일링합니다.
                        </p>
                    </div>
                    {phase > 0 && (
            ...
            {phase >= 2 && (
                <SectionCard padding="p-6" className="animate-in">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <div className="flex items-center gap-2 text-slate-800 font-bold text-lg">
                                <div className="w-8 h-8 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center"><Zap size={18}/></div>
                                Phase 2 · 서브 에이전트 실행 계획
                            </div>
                            <p className="text-xs text-slate-500 mt-1 ml-10">
                                넥슨스킬즈 카탈로그에서 매칭된 스킬들을 서브 에이전트가 실행합니다.
                                <span className="ml-2 inline-flex items-center gap-1 text-violet-600 font-bold"><Package size={10}/> 카탈로그</span>
                                <span className="mx-1 text-slate-300">|</span>
                                <span className="inline-flex items-center gap-1 text-amber-600 font-bold"><Zap size={10}/> 인스턴트</span>
                            </p>
                        </div>
                        <GameButton onClick={handleExecute} disabled={phase >= 3} color="amber" size="sm">
                            {phase >= 4 ? <><Check size={16}/> 실행 완료</> :
                             phase === 3 ? <><Loader2 size={16} className="animate-spin"/> 실행 중...</> :
                    ...
                    <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                        {plannedSkills.map((item) => {
                            ...
                            return (
                                <button key={item.id} onClick={() => setSelectedNode(item)}
                                    className={`text-left p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-md ${
                                        isExecuted ? tBorder.done : 'border-slate-200 bg-white ' + tBorder.idle
                                    }`}>
                                    <div className="flex items-center justify-between">
                                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${
                                            isExecuted ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'
                                        }`}>
                                            {isExecuted ? <CheckCircle2 size={18}/> : <Icon size={18}/>}
                                        </div>
                                    ...
                                    {isExecuted && (
                                        <div className="mt-2 text-[11px] text-emerald-700 font-bold truncate bg-emerald-50 border border-emerald-100 rounded-lg px-2 py-1">{item.output}</div>
                                    )}
                                </button>
                            );
                        })}
                    ...
                    {phase >= 4 && (
                        <div className="mt-4 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5 flex items-center gap-2">
                            <CheckCircle2 size={14}/> 전체 서브 에이전트 실행 완료 · 아래 DIFY GUI에서 결과를 확인하고 디테일링하세요.
                        </div>
                    )}
                </SectionCard>
            )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <div className="flex items-center gap-2 text-slate-800 font-bold text-lg">
                                ...
                                Phase 3 · DIFY GUI 디테일링
                            </div>
                            <p className="text-xs text-slate-500 mt-1 ml-10">생성된 워크플로우를 시각적으로 확인하고 세부 조정합니다.</p>
                        </div>
                    ...
                    {showYamlView ? (
                        ...
                        <div className={`mt-4 border border-slate-200 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-blue-50/30 overflow-hidden relative ${isPanning ? 'cursor-grabbing' : 'cursor-grab'}`}
                            ...
                            <div className="absolute top-3 left-4 z-10 text-[11px] font-bold text-slate-400 flex items-center gap-2">
                                ...
                                <span className="bg-white/80 backdrop-blur px-2.5 py-1 rounded-lg border border-slate-200">드래그로 이동 · 노드 클릭 → 상세</span>
                            </div>
                            ...
                            <div className="absolute left-0 top-0 w-[1560px] h-[400px]" style={{ transform: `translate(${canvasOffset.x}px, ${canvasOffset.y}px)` }}>
                                ...
                                {plannedSkills.map((item) => {
                                    ...
                                    const tBorder = typeNodeBorder[item.skillType] || typeNodeBorder.Agent;
                                    return (
                                        <button key={item.id} data-node-card="true" type="button" onClick={() => setSelectedNode(item)}
                                            className={`absolute text-left border-2 rounded-2xl p-3 w-[220px] shadow-sm hover:shadow-lg transition-all duration-300 ${isExecuted ? tBorder.done : 'bg-white ' + tBorder.idle}`}
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                    <div className="flex-1 overflow-y-auto p-6">
                        <div className="max-w-4xl mx-auto space-y-2">
                            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 px-1">Phase 1 → 2 전체 파이프라인 로그</div>
        ...
        {selectedNode && (() => {
            ...
            const isExecuted = executedNodes.has(selectedNode.id);
            ...
            return (
                <FullScreenPanel className="z-[100]">
                    <PanelToolbar
                        ...
                        subtitle="AI 실험실 / 노드 상세"
                        title={selectedNode.skillName}
                        statusBadge={<>
                            <TypeBadge type={selectedNode.skillType} size="xs" />
                            {isExecuted && <Badge color="emerald">Executed</Badge>}
                        </>}
                        actions={<GameButton onClick={() => setSelectedNode(null)} color="slate" size="sm">닫기</GameButton>}
                    />
                    <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                            ...
                            <div className="max-w-3xl mx-auto space-y-6">
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                                    <div className={`px-6 py-5 border-b border-slate-100 flex items-center gap-4 ${
                                        ...
                                        }`}><NodeIcon size={28}/></div>
                                        <div>
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <Badge color="violet">{selectedNode.id}</Badge>
                                                {selectedNode.catalogId ? (
                                                    <span className="text-[10px] font-bold text-violet-600 bg-violet-50 border border-violet-200 px-2 py-0.5 rounded-md flex items-center gap-1"><Package size={10}/> {selectedNode.catalogId}</span>
                                                ) : (
                                                    <span className="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md flex items-center gap-1"><Zap size={10}/> 인스턴트</span>
                                                )}
                                            </div>
                                            <h2 className="text-2xl font-bold text-slate-900 mt-1">{selectedNode.skillName}</h2>
                                            <p className="text-sm text-slate-500">{selectedNode.originLabel}</p>
                                        </div>
                                    </div>
                                    <div className="p-6 space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                                                ...
                                                <div className="mt-1 font-mono text-slate-800 text-sm">{selectedNode.skill}</div>
                                            </div>
                                            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                                                <div className="text-xs font-semibold text-slate-500">라우팅 경로</div>
                                                <div className="mt-1 font-mono text-slate-800 text-sm break-all">{selectedNode.route}</div>
                                            </div>
                                        </div>
                                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                                            <div className="text-xs font-semibold text-slate-500">노드 설명</div>
                                            <div className="mt-1.5 text-slate-700 leading-relaxed">{selectedNode.detail}</div>
                                        </div>
                                        {isExecuted && (
                                            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                                                <div className="text-xs font-semibold text-emerald-700">실행 결과</div>
                                                <div className="mt-1.5 text-emerald-900 font-bold text-lg">{selectedNode.output}</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                                    <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2 mb-4"><Workflow size={16}/> 연결된 노드</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {upstreamNodes.length > 0 && (
                                            <div>
                                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Upstream (입력)</div>
                                                <div className="space-y-2">
                                                    {upstreamNodes.map(node => {
                                                        const UpIcon = node.icon;
                                                        return (
                                                            <button key={node.id} onClick={() => setSelectedNode(node)} className="w-full text-left flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50 hover:border-violet-300 hover:bg-violet-50/30 transition-all">
                                                                <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center"><UpIcon size={14}/></div>
                                                                <div className="min-w-0">
                                                                ...
                                                                <TypeBadge type={node.skillType} size="xs" className="ml-auto" />
                                                            </button>
                                                        );
                                                    })}
                                                </div>
                                        ...
                                        {downstreamNodes.length > 0 && (
                                            <div>
                                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Downstream (출력)</div>
                                                <div className="space-y-2">
                                                    {downstreamNodes.map(node => {
                                                        const DownIcon = node.icon;
                                                        return (
                                                            <button key={node.id} onClick={() => setSelectedNode(node)} className="w-full text-left flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50 hover:border-violet-300 hover:bg-violet-50/30 transition-all">
                                                                <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center"><DownIcon size={14}/></div>
                                                                <div className="min-w-0">
                                                                    <div className="text-xs font-bold text-slate-800 truncate">{node.skillName}</div>
                                                                    <div className="text-[10px] text-slate-400 font-mono truncate">{node.id}</div>
                                                                </div>
                        ...
                        <div className="hidden lg:flex flex-col w-96 bg-white border-l border-slate-200">
                            <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
                                <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2"><Code size={16}/> 노드 YAML 정의</h4>
                            </div>
                            <pre className="flex-1 overflow-auto p-4 text-xs text-slate-600 font-mono leading-relaxed whitespace-pre-wrap bg-slate-50">{[
                                `- id: ${selectedNode.id}`, '  data:', '    type: custom',
                                `    title: "${selectedNode.skillName}"`, `    desc: "${selectedNode.detail}"`,
                                `    skill: "${selectedNode.route}"`, `    nexon_catalog_id: "${selectedNode.catalogId || 'instant'}"`,
                                `    skill_type: "${selectedNode.skillType}"`, '    selected: false', '    variables: []',
                                '  position:', `    x: ${nodePositions[selectedNode.id]?.x ?? 0}`, `    y: ${nodePositions[selectedNode.id]?.y ?? 0}`,
                            ].join('\n')}</pre>
                        </div>
                    </div>
                </FullScreenPanel>
