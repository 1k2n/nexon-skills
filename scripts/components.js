...
const { GAME_LIST, MAIN_MENUS, NEWS_ITEMS, FAVORITES_ITEMS, HOME_INITIAL_ITEMS } = window.APP_DATA;

const resolveMenuIcon = (icon) => (typeof icon === 'string' ? window.Lucide[icon] : icon);
...
className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all ${
    ...
    >
{(() => { const Icon = resolveMenuIcon(menu.icon); return Icon ? <Icon size={18} className={currentTab === menu.name ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'} strokeWidth={2.5}/> : null; })()}
{menu.name}
...
<select value={selectedGame} onChange={(e) => setSelectedGame(e.target.value)} className="bg-transparent text-sm font-bold text-slate-700 outline-none cursor-pointer max-w-[100px] truncate hover:text-blue-600">
    ...
    <input type="text" placeholder="소환하고 싶은 스킬 검색..." className="bg-transparent text-sm w-full focus:outline-none font-medium placeholder-slate-400" />
...
<div className="flex items-center gap-3">{(() => { const Icon = resolveMenuIcon(menu.icon); return Icon ? <Icon size={20} strokeWidth={2.5} /> : null; })()} {menu.name}</div>
...
<button onClick={() => { setIsMenuOpen(false); setShowLoginScreen(true); }} className="flex items-center justify-center gap-2 py-4 bg-slate-900 text-white rounded-2xl font-bold"><LogIn size={20} strokeWidth={2.5} /> 로그인</button>
    )}
</div>
    </div>
</div>
...
// =================================================================================================
// [SECTION 4] FEATURE COMPONENTS (기능별 컴포넌트)
// =================================================================================================
